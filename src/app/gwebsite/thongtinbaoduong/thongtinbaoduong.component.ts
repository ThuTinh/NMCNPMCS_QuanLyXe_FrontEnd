...
import { ThongTinBaoDuongServiceProxy, ModelForViewDto, CheckServiceProxy, ThongTinBaoDuongDto, UserSevicrServiceProxy } from '@shared/service-proxies/service-proxies';
...
export class ThongTinBaoDuongComponent extends AppComponentBase implements AfterViewInit, OnInit {
    ...
    currentUser: string;
    thongtinbaoduongs: ThongTinBaoDuongDto[] = [];
    ...
    reloadList(thongtinbaoduongSoXe, event?: LazyLoadEvent) {
        ...
        if (this.soXe != undefined) {
            this._thongtinbaoduongService.getThongTinBaoDuongsByFilter(this.soXe, this.primengTableHelper.getSorting(this.dataTable),
                this.primengTableHelper.getMaxResultCount(this.paginator, event),
                this.primengTableHelper.getSkipCount(this.paginator, event),
            ).subscribe(result => {
                result.items.map(item => {
                    if (item.trangThaiDuyet == "Đã duyệt" || item.createdBy == this.currentUser) {
                        this.thongtinbaoduongs.push(item);
                    }
                })
                if (this.isDuyet) {
                    ...
                    this.primengTableHelper.totalRecordsCount = this.thongtinbaoduongs.length;
                    this.primengTableHelper.records = this.thongtinbaoduongs
                }
